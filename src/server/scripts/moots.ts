...
export async function refreshMutual(username: string, getHeader?: boolean): Promise<(UserV2 & { header: ProfileBannerSizeV1 }) | void>
export async function refreshMutual(username: string): Promise<unknown> {
    if (!bearer_token) {
        return console.warn("No bearer token provided. refreshMutual() returned.");
    }

    const client = new TwitterApi(bearer_token).readOnly;
    const usernameLookup = await client.v2.userByUsername(
        username,
        {
            "user.fields": [
                "profile_image_url",
    ...
    if (usernameLookup.errors?.length)
        return;
...
export async function refreshMootsList(config?: RefreshMootsListConfig) {
        ...
        if (moot.id == "-1") {
            ...
            if (refreshedData) {
                ...
                moots_list[i] = {
                    ...
                    username, 
                    header: "",
                    icon: profile_image_url ? profile_image_url.replace("normal", "bigger") : "",
                };
            }
        }
    }
    ...
    await fs.writeFile(filePath, JSON.stringify(moots_list, null, 4), { encoding: "utf-8" });
    return; 
}